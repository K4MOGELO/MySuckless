...
# define S_FORMAT_CLEAR "sed 's/^..//'"
	...
	if (strcmp (exit_action, S_LOCK) == 0) system ("i3lock");
	...
	else if (strcmp (exit_action, S_REBOOT) == 0) system ("sleep 2; sreboot");
	else if (strcmp (exit_action, S_SHUTDOWN) == 0) system ("sleep2; sshutdown");
